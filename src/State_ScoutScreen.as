package  
{
	...
	public class State_ScoutScreen extends CGameState
	{
		...
		private var m_ScreenScout:mc_ScoutScreen;
		...
		private static const UZTAN:int 			= 1;
		private static const RAZARK:int 		= 2;
		private static const CROZTAN:int		= 3;
		private static const HESTACLAN:int		= 4;
		private static const TEECLON:int		= 5;
		private static const UDIZARK:int		= 6;
		private static const UMAZ:int			= 7;
		private static const RAMMER:int			= 8;
		private static const CAPLOZTON:int		= 9;
		private static const BALISTATOZ:int		= 10;
		private static const POZTAZARK:int		= 11;
		private static const WEEZEE:int			= 12;
		private static const TEEGOOR:int		= 13;
		...
		override public function initialize(owner:DisplayObjectContainer):void 
		{
			...
			//adding new object			
			m_ScreenScout = new mc_ScoutScreen();
			
		/*	wave = [[0],[1], [1], [1, 3], [1, 3],
					[1, 3, 2], [1, 3, 2], [1, 3, 2, 7], [1, 3, 2, 7],
					[1, 3, 2, 7 ,5], [1, 3, 2, 7 ,5], [1, 3, 2, 7 ,5, 4], [1, 3, 2, 7 ,5, 9],
					[1, 3, 2, 7 ,4, 9, 11], [1, 3, 2, 7 , 4, 9, 11], [1, 3, 2, 7 ,9, 4, 11], [1, 3, 2, 7 , 4, 11,8],
					[1,7, 4, 11, 5, 10], [1, 3, 2, 7 , 5, 4, 9, 11, 8, 10, 6], [1, 3, 2, 7 , 5, 4, 9, 11, 8, 10, 6, 12], [1, 3, 2, 7 , 5, 4, 9, 11, 8, 10, 6, 12],
					[1, 3, 2, 7 , 5, 4, 9, 11, 8, 10, 6, 12,13]];*/
			wave = [[0], [UZTAN], [UZTAN], [UZTAN, CROZTAN], [UZTAN, CROZTAN], [UZTAN, CROZTAN, RAZARK], 
						[UZTAN, CROZTAN, RAZARK], [UZTAN, CROZTAN, RAZARK, UMAZ], [UZTAN, CROZTAN, RAZARK, UMAZ],[UZTAN, CROZTAN, RAZARK, UMAZ, TEECLON], [UZTAN, CROZTAN, RAZARK, UMAZ, TEECLON], 
						[UZTAN, CROZTAN, RAZARK, UMAZ, TEECLON, HESTACLAN], [UZTAN, CROZTAN, RAZARK, UMAZ, TEECLON, CAPLOZTON], [UZTAN, CROZTAN, RAZARK, UMAZ, CAPLOZTON, HESTACLAN, POZTAZARK], [UZTAN, CROZTAN, RAZARK, UMAZ, HESTACLAN, POZTAZARK,RAMMER], [UZTAN, UMAZ, HESTACLAN, POZTAZARK,TEECLON,BALISTATOZ], 
						[UZTAN, CROZTAN, RAZARK, UMAZ, TEECLON, HESTACLAN, POZTAZARK, CAPLOZTON, UDIZARK], [UZTAN, CROZTAN, RAZARK, UMAZ, TEECLON, HESTACLAN, POZTAZARK, CAPLOZTON, UDIZARK, WEEZEE], [UZTAN, CROZTAN, RAZARK, UMAZ, TEECLON, HESTACLAN, CAPLOZTON, POZTAZARK, RAMMER, BALISTATOZ, UDIZARK], [UZTAN, CROZTAN, RAZARK, UMAZ, TEECLON, HESTACLAN, CAPLOZTON, POZTAZARK, RAMMER, BALISTATOZ, UDIZARK, WEEZEE],
						[UZTAN, CROZTAN, RAZARK, UMAZ, TEECLON, HESTACLAN, CAPLOZTON, POZTAZARK, RAMMER, BALISTATOZ, UDIZARK, WEEZEE],
						[UZTAN, CROZTAN, RAZARK, UMAZ, TEECLON, HESTACLAN, CAPLOZTON, POZTAZARK, RAMMER, BALISTATOZ, UDIZARK, WEEZEE,TEEGOOR]]
			...
			game_tips = [["Build a Treasury to help you gain more gold","Teemy can bash enemies on first encounter and throw them backward (this only can be done once)", "Croztan is weak against Feela, so don't forget to recruit them","Remember to always place your troops on all lanes of defense","Your units will level up after several kills"],
						 ["The wave meter at the right-bottom shows level progress", "Retreat your units to give you back gold , half-price of it", "You can only swap your troops on the same lanes","Leegos is strong againts enemy cavalry. Use this advantage to beat them","Seeldy can block enemies to prevent them too close to our palace"],
						 ...
						 ["You can disband your troops and get back your research points", "Spear attack is very helpful on this attack", "Don't forget to check both your troops and enemy units weakness", "Trap instantly kill any enemy units that pass it,but destroyed after 3 attacks", "Barricade is Umaz worst nightmare.."],
						 ["DANGER!!!!"]];
		...
		override public function enter():void 
		{
			...
			//add to stage
			m_owner.addChild(m_ScreenScout);
			
			m_ScreenScout.bg_scoutscr.gotoAndStop(GlobalVars.CURRENT_LEVEL + 1);
			m_ScreenScout.panel.tips_text.text = game_tips[GlobalVars.CURRENT_LEVEL][GlobalVars.CURRENT_SUB_LEVEL];
		
			m_ScreenScout.panel.scaleX = m_ScreenScout.panel.scaleY = 0;
			m_ScreenScout.panel.ok_btn.gotoAndStop(1);
			m_ScreenScout.panel.cheatBtn.gotoAndStop(1);
			m_ScreenScout.panel.moregamesBtn.gotoAndStop(1);
			
			if (!GlobalVars.HACK)
			{
				//TweenMax.to(m_ScreenScout.cheatBtn , 0.25, {delay:0.3, x:m_ScreenScout.cheatBtn.x, y:-40, ease:Quint.easeOut } );
				m_ScreenScout.panel.cheatBtn.addEventListener(MouseEvent.ROLL_OVER, onMouseOver);
				m_ScreenScout.panel.cheatBtn.addEventListener(MouseEvent.ROLL_OUT, onMouseLeave);
				m_ScreenScout.panel.cheatBtn.addEventListener(MouseEvent.MOUSE_DOWN, onMouseClick);				
				m_ScreenScout.panel.cheatBtn.addEventListener(MouseEvent.MOUSE_UP, playCheatMode);
			}
			
			//TweenMax.to(m_ScreenScout.moregamesBtn , 0.25, {delay:0.3 , x: m_ScreenScout.moregamesBtn.x, y:-40, ease:Quint.easeOut } );
			m_ScreenScout.panel.moregamesBtn.addEventListener(MouseEvent.ROLL_OVER, onMouseOver);
			m_ScreenScout.panel.moregamesBtn.addEventListener(MouseEvent.ROLL_OUT, onMouseLeave);
			m_ScreenScout.panel.moregamesBtn.addEventListener(MouseEvent.MOUSE_DOWN, onMouseClick);
			m_ScreenScout.panel.moregamesBtn.addEventListener(MouseEvent.MOUSE_UP, playMoreGames);
			
			TweenMax.to(m_ScreenScout.podium_grass , 0.25, { x:-28, y:418, ease:Quint.easeOut , onComplete:function():void {
				TweenMax.to(m_ScreenScout.podium , 0.25, { x:0, y:m_ScreenScout.podium.y, ease:Quint.easeOut , onComplete:function():void {
					TweenMax.to(m_ScreenScout.panel , 0.25, {scaleX:1, scaleY:1 , ease:Quint.easeOut } );
					}});
			}})
			...
			for (var i:int = 1 ; i <= 13 ; i++)
			{
				...
				m_ScreenScout.panel["unit_" + i].hitarea.alpha = 0;
				m_ScreenScout.panel["unit_" + i].visible = false;
			...
			toggleCustomCursor();
			//register button
			registerButton(m_ScreenScout.panel.ok_btn);
		...
		{
			unregisterButton(m_ScreenScout.panel.ok_btn);
			stage.removeChild(m_customCursor);
			m_bgm.stop();
			m_owner.removeChild(m_ScreenScout);	
			super.exit();
		}
		...
		private function show_wave():void
		{
			...
			for (var i:int = 0; i < len ; i++)
			{
				...
				m_ScreenScout.panel["unit_" + wave[level][i]].id = unit_id;
				m_ScreenScout.panel["unit_" + wave[level][i]].visible = true;
				m_ScreenScout.panel["unit_" + wave[level][i]].gotoAndStop(unit_id);
				m_ScreenScout.panel["unit_" + wave[level][i]].addEventListener(MouseEvent.ROLL_OVER, onMouseOver);
				m_ScreenScout.panel["unit_" + wave[level][i]].addEventListener(MouseEvent.ROLL_OUT, onMouseLeave);
			}
			...
			if (wave[level][len - 1] != wave[level - 1][prevLen - 1] && level > 1)
			{
				...
				m_ScreenScout.panel.addChild(sign);
				sign.x = m_ScreenScout.panel["unit_" + wave[level][(len - 1)]].x;
				sign.y = (m_ScreenScout.panel["unit_" + wave[level][(len - 1)]].y - m_ScreenScout.panel["unit_" + wave[level][(len - 1)]].width);
				TweenMax.to(sign, 0.5, {glowFilter:{color:0xffcc00, alpha:1, blurX:30, blurY:30,quality:2}});
				TweenMax.to(m_ScreenScout.panel["unit_" + wave[level][(len - 1)]], 0.5, { glowFilter: { color:0xffcc00, alpha:1, blurX:30, blurY:30, strength:1.5, quality:2 }} );
			}
			/*else
			{
				if (sign != null)
		...
		private function onMouseClick(event:MouseEvent):void
		{
			...
			switch(mc)
			{
				case m_ScreenScout.panel.ok_btn:
				mc.gotoAndStop(2);
				break;
				case m_ScreenScout.panel.moregamesBtn:
				mc.gotoAndStop(2);
				break;
				case m_ScreenScout.panel.cheatBtn:
				mc.gotoAndStop(2);
				break;
			}
		...
		private function onMouseOver(event:MouseEvent):void
		{
			...
			switch (mc)
			{
				...
				m_ScreenScout.panel.addChild(description);
				description.mouseChildren = false;
				description.mouseEnabled = false;
				if (mc.x > 400)
				{
					description.x = mc.x - 50;
					description.y = mc.y;
				}
				else
				{
		...
		private function onMouseLeave(event:MouseEvent):void
		{
			if (description != null)
			{
				m_ScreenScout.panel.removeChild(description);
				description = null;
			}
			//trace("rollout");
		...
		private function onMouseUp(event:MouseEvent):void
		{
			...
			switch(mc)
			{
				...
				if (sign != null)
				{
					trace("DELETE");
					TweenMax.to(sign, 0.5, {glowFilter:{color:0xffcc00, alpha:0, blurX:30, blurY:30,quality:2}});
					m_ScreenScout.panel.removeChild(sign);
					sign = null;
					TweenMax.to(m_ScreenScout.panel["unit_" + wave[level][(len - 1)]], 0.5, { glowFilter: { color:0xffcc00, alpha:0, blurX:30, blurY:30, strength:1.5, quality:2 }} );
				}
				mc.gotoAndStop(1);
				
				if (!GlobalVars.HACK)
				{
					//TweenMax.to(m_ScreenScout.cheatBtn , 0.25, {delay:0.1, x:m_ScreenScout.cheatBtn.x, y:-100, ease:Quint.easeOut } );
					m_ScreenScout.panel.cheatBtn.removeEventListener(MouseEvent.MOUSE_UP, playCheatMode);
				}
			
				//TweenMax.to(m_ScreenScout.moregamesBtn , 0.25, {delay:0.1 , x: m_ScreenScout.moregamesBtn.x, y:-100, ease:Quint.easeOut } );
				m_ScreenScout.panel.moregamesBtn.removeEventListener(MouseEvent.MOUSE_UP, playMoreGames);
			
				TweenMax.to(m_ScreenScout.panel , 0.25, { scaleX:0, scaleY:0 , ease:Quint.easeOut, onComplete:function():void {
				TweenMax.to(m_ScreenScout.podium , 0.25, { x:-300, y:0, ease:Quint.easeOut , onComplete:function():void {
					TweenMax.to(m_ScreenScout.podium_grass , 0.5, { x:-28, y:500, ease:Quint.easeOut , onComplete:function():void {
