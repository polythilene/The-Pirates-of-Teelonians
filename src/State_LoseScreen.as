package  
{
	...
	public class State_LoseScreen extends CGameState
	{
		...
		private var m_ScreenLose:mc_LoseScreen;
		...
		override public function initialize(owner:DisplayObjectContainer):void 
		{
			...
			//adding new object
			m_ScreenLose = new mc_LoseScreen();
		...
		override public function enter():void 
		{
			...
			m_owner.addChild(m_ScreenLose);
			
			m_ScreenLose.panel.scaleX = m_ScreenLose.panel.scaleY = 0;
			m_ScreenLose.panel.accept_btn.gotoAndStop(1);
			m_ScreenLose.panel.decline_btn.gotoAndStop(1);
			
			if (!GlobalVars.HACK)
			{
				TweenMax.to(m_ScreenLose.cheatBtn , 0.25, {delay:0.3, x:m_ScreenLose.cheatBtn.x, y:-40, ease:Quint.easeOut } );
				m_ScreenLose.cheatBtn.addEventListener(MouseEvent.MOUSE_UP, playCheatMode);
			}
			
			TweenMax.to(m_ScreenLose.moregamesBtn , 0.25, {delay:0.3, x: m_ScreenLose.moregamesBtn.x, y:-40, ease:Quint.easeOut } );
			m_ScreenLose.moregamesBtn.addEventListener(MouseEvent.MOUSE_UP, playMoreGames);		
			
			TweenMax.to(m_ScreenLose.podium_grass , 0.25, { x:-28, y:418, ease:Quint.easeOut , onComplete:function():void {
				TweenMax.to(m_ScreenLose.podium , 0.25, { x:0, y:m_ScreenLose.podium.y, ease:Quint.easeOut , onComplete:function():void {
					TweenMax.to(m_ScreenLose.panel , 0.25, {scaleX:1, scaleY:1 , ease:Quint.easeOut } );
					}});
			}})
			...
			m_ScreenLose.panel.donation_gold.text = String(donationGold);
			m_ScreenLose.panel.current_gold.text = String(currentGold);
			
			
			registerButton(m_ScreenLose.panel.accept_btn);
			registerButton(m_ScreenLose.panel.decline_btn);
		...
		{
			m_bgm.stop();
			m_owner.removeChild(m_ScreenLose);
			super.exit();
		}
		...
		private function onMouseUp(event:MouseEvent):void
		{
			...
			switch (mc)
			{
				case m_ScreenLose.panel.accept_btn:
				GlobalVars.TOTAL_COIN = donationGold;
				
				if (!GlobalVars.HACK)
				{
					TweenMax.to(m_ScreenLose.cheatBtn , 0.25, {delay:0.1, x:m_ScreenLose.cheatBtn.x, y:-100, ease:Quint.easeOut } );
					m_ScreenLose.cheatBtn.addEventListener(MouseEvent.MOUSE_UP, playCheatMode);
				}
			
				TweenMax.to(m_ScreenLose.moregamesBtn , 0.25, {delay:0.1 , x: m_ScreenLose.moregamesBtn.x, y:-100, ease:Quint.easeOut } );
				m_ScreenLose.moregamesBtn.addEventListener(MouseEvent.MOUSE_UP, playMoreGames);
				
				TweenMax.to(m_ScreenLose.panel , 0.25, { scaleX:0, scaleY:0 , ease:Quint.easeOut, onComplete:function():void {
				TweenMax.to(m_ScreenLose.podium , 0.25, { x:-300, y:0, ease:Quint.easeOut , onComplete:function():void {
					TweenMax.to(m_ScreenLose.podium_grass , 0.25, { x:-28, y:500, ease:Quint.easeOut , onComplete:function():void {
						GameStateManager.getInstance().setState(State_BarrackScreen.getInstance());
						}} );
					}});
				}})
				break;
				case m_ScreenLose.panel.decline_btn:
