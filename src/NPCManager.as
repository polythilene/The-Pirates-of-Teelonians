...
package
{
	...
	import com.game.CCaptainBakuba;
	import com.game.CCaptainJack;
	import com.game.CCaptainSmith;
	...
	public class NPCManager extends EventDispatcher
	{
		...
		{
			/*var id:String
			if (npc is CPlayerTeelos || npc is CPlayerCaptain)
			{
				id = FACTION_PLAYER;
			}
			else
			...
			if ( id == FACTION_PLAYER )
			{
				...
				if ( ( npc.isDestinationReached() || npc is CTeeloBaseSummoned || 
					   npc is CTeeloTeemy || npc is CTeeloLastBarricade || npc is CPlayerCaptain ) && enemyList.length > 0  )
				{
					
					for ( var i:int = 0; i < enemyList.length; i++ )
					{
						var enemy:CBaseTeelos = enemyList[i];
						if (npc is CCaptainJack || npc is CCaptainBakuba || npc is CCaptainSmith)
						{
							if ( (npc.x < enemy.x && !enemy.isDead() && (enemy.x - npc.x) < nearest && enemy.x < 800) && npc.laneIndex == enemy.laneIndex)
							{
								ret = enemy;
		...
		private function unregisterFromLane(npc:CBaseTeelos):void
		{
			...
			if ( npc is CTeeloTeegor )
			{
				for ( var i:int = 0; i < 5; i++ )
				{
					var index:int = m_lane[i][id].indexOf(npc);
					 m_lane[i][id].splice(index, 1);
				}
			}
			else
			{
				index = m_lane[npc.laneIndex][id].indexOf(npc);
				if( index != -1 )
